...
public class Robot extends TimedRobot {
  ...
  Servo odomHitecServo = new Servo(8);
  ...
  public void testPeriodic() {
    ...
    // read and print the state of an external button board
    ...
    // read a Sharp IR sensor (see WPLib "Hardware >> Sensors >> Analog Inputs - Software"):
    ...
    // move hitec servo with IR sensor:
    while(true) {
      irValueAvg = odomIRRanger.getValue(); // gets the unaveraged value from the sensor
      System.out.println("irValueAvg = " + irValueAvg);
      
      if (irValueAvg > 700) {
        odomLED.set(false);
        odomHitecServo.setAngle(0);
      }
      else if(irValueAvg > 200) {
        odomLED.set(false);
        odomHitecServo.setAngle(180);
      }
      else {
        odomLED.set(true);
        odomHitecServo.setAngle(90);
      }
    }
